...
**テストケース TC-ENGINE-016**: 基礎控除の段階制（2020年～2024年）
- **前提条件**: 2020年～2024年、総所得2,400万円以下
- **入力**: `totalIncomeGeneral = 24,000,000`
- **期待結果**: 基礎控除 = 480,000円
- **検証項目**: 正しい段階が選択されること

**テストケース TC-ENGINE-016-2**: 基礎控除の段階制（2025年以降）
- **前提条件**: 2025年以降、総所得2,350万円以下
- **入力**: `totalIncomeGeneral = 23,500,000`
- **期待結果**: 基礎控除 = 580,000円
- **検証項目**: 正しい段階が選択されること

**テストケース TC-ENGINE-016-3**: 基礎控除の段階制（2025年以降、2,350万円超～2,400万円以下）
- **前提条件**: 2025年以降、総所得2,350万円超～2,400万円以下
- **入力**: `totalIncomeGeneral = 24,000,000`
- **期待結果**: 基礎控除 = 480,000円
- **検証項目**: 正しい段階が選択されること

**テストケース TC-ENGINE-016-4**: 基礎控除（2019年以前）
- **前提条件**: 2019年以前
- **入力**: `totalIncomeGeneral = 任意の値`
- **期待結果**: 基礎控除 = 380,000円（一律）
- **検証項目**: 所得金額に関わらず一律38万円

**テストケース TC-ENGINE-017**: 基礎控除の段階縮小（高所得者）
- **前提条件**: 2020年～2024年、総所得2,500万円超
- **入力**: `totalIncomeGeneral = 25,000,001`
- **期待結果**: 基礎控除 = 0円
- **検証項目**: 高所得者で控除が0になること
...
**テストケース TC-ENGINE-025-2**: 所得税（合計）100円未満切り捨て
- **前提条件**: 所得税（総合課税）、分離課税の所得税、復興特別所得税
- **入力**: `incomeTax = 123,456`, `separateIncomeTax = 40,678`, `separateReconstructionTax = 1,234`（合計165,368円）
- **期待結果**: 所得税（合計） = 165,300円
- **検証項目**: 100円未満が切り捨てられること
...
**テストケース TC-ENGINE-026-2**: 所得税（合計）100円未満切り捨て
- **前提条件**: 所得税（総合課税）、分離課税の所得税、復興特別所得税
- **入力**: `incomeTax = 123,456`, `separateIncomeTax = 40,678`, `separateReconstructionTax = 1,234`（合計165,368円）
- **検証項目**: `incomeTaxTotal = 165,300`（100円未満切り捨て）

**テストケース TC-ENGINE-027**: 住民税（所得割）
- **前提条件**: 課税所得金額479万円、所得割率10%
- **入力**: `residentTaxableIncome = 4,790,000`, `income_rate = 0.1`
- **期待結果**: 所得割 = 479,000円
- **検証項目**: `residentIncomePart = floor(floor(residentTaxableIncome * income_rate) / 100) * 100`（100円未満切り捨て）

**テストケース TC-ENGINE-027-2**: 住民税（所得割）100円未満切り捨て
- **前提条件**: 課税所得金額、所得割率
- **入力**: `residentTaxableIncome = 4,791,234`, `income_rate = 0.1`（計算結果479,123.4円）
- **期待結果**: 所得割 = 479,100円
- **検証項目**: 100円未満が切り捨てられること
...
**テストケース TC-UI-002**: 必須項目未入力で「入力内容で計算」を押下すると、先頭エラーへスクロール
